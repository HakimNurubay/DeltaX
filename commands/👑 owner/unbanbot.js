...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (message.author.id == owner) {
      ...
      if (!user) return message.channel.send("Plz mention a someone to unbanned from DeltaX.");
      let reason = args.slice(1).join(" ");
      if (!reason) reason = "No Reason Provided";
      const Blacklisted = db.fetch(`blacklistedUser_${user.id}`);
      if (Blacklisted == false) return message.channel.send("This user is already unbanned from DeltaX.");
      message.channel.send(`Successfully unbanned ${user.username} from DeltaX.`);
      db.set(`blacklistedUser_${user.id}`, false);

      let embed = new Discord.MessageEmbed().setDescription(`${check} You have been unbanned from DeltaX Bot.\nReason: ${reason}`).setColor("#FFFF00");
    ...
    const banlogEmbed = new Discord.MessageEmbed()
      ...
      .setColor("RED")
      .setThumbnail(Avatar)
      .addFields({ name: "Unbanned ID", value: `${target.id}`, inline: true }, { name: "Unbanned Tag", value: `${target.tag}`, inline: true });

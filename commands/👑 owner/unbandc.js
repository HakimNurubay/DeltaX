  ...
  run: async (client, message, args) => {
    ...
    if (message.author.id == owner) {
      ...
      if (guild) {
        ...
        if (Blacklisted == false) return message.channel.send("This Guild is already unbanned from DeltaX.");
        message.channel.send(`Successfully banned ${guild.name} from DeltaX.`);
        db.set(`blacklistedServer_${guild.id}`, false);

        const icon = guild.iconURL() || "https://cdn.discordapp.com/embed/avatars/1.png";

        const banlogEmbed = new Discord.MessageEmbed()
          .setTitle("New Unbanned Server!")
          .setDescription(`Server: \`${guild.name}\``)
          .setColor("RED")
          .setThumbnail(icon)
          .addFields({ name: "Unbanned ID", value: `${guild.id}`, inline: true });

...
module.exports = {
  name: "set-leave-background",
  aliases: [],
  usage: "<link .png/.jpg/.jpeg>",
  category: "🚪 leave",
  description: "This command will set ur leave logs background",
  run: async (client, message, args) => {
    if (cooldown.has(message.author.id)) {
      ...
      if (message.member.hasPermission("MANAGE_CHANNELS", "ADMINISTRATOR")) {
        ...
        message.channel.send(`Successfully set the leave background.`);
        await db.set(`backgroundL_${message.guild.id}`, args[0]);
      } else {
